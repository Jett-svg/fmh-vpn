...
# ========== FLASK ДЛЯ RENDER (health-check и вебхуки) ==========
...
def create_user(user_id):
    try:
        conn = get_db_connection()
        c = conn.cursor()
        # MySQL: ON CONFLICT (Postgres) заменён на INSERT IGNORE — user_id
        # это PRIMARY KEY, так что при дубле строка просто не вставится
        # (аналог "DO NOTHING").
        c.execute('''
            INSERT IGNORE INTO users (user_id, subscription_end, devices, max_devices, bonus_balance, phone, first_time)
...
def payment_tariff_menu_with_bonus(user_id, selected_tariff=None, selected_plan=None):
    ...
    if selected_tariff and selected_plan:
        ...
        keyboard = []
        if bonus >= price:
            keyboard.append([InlineKeyboardButton(f"💰 Оплатить ВСЕ бонусами ({price} ₽)",
                                                  callback_data=f"bonus_all_{selected_tariff}_{selected_plan}")])
        if bonus > 0:
            keyboard.append([InlineKeyboardButton(f"🔄 Частично бонусами (есть {bonus} ₽)",
                                                  callback_data=f"bonus_partial_input_{selected_tariff}_{selected_plan}")])
        keyboard.append([InlineKeyboardButton(f"💳 Полностью деньгами — {price} ₽",
...
async def send_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if subscription_end:
        ...
        if end_date and end_date > datetime.now():
            ...
            status = "✅ Активна"
            end_text = f"до {end_date.strftime('%d.%m.%Y')} (осталось {days_left} дн.)" if days_left > 0 else f"до {end_date.strftime('%d.%m.%Y')} (осталось {hours_left} ч.)"
        else:
            status = "❌ Истекла"
...
async def payment_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_data and user_data[4]:  # Если телефон указан
        await update.effective_chat.send_message(
            "💸 **Выберите тариф:**\n\n" f"💰 Ваш бонусный счет: **{user_data[3]} ₽**",
            parse_mode='Markdown', reply_markup=payment_tariff_menu_with_bonus(user_id)
        )
...
async def payment_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if success:
        ...
        await update.message.reply_text("❌ Ошибка сохранения номера. Попробуйте позже или нажмите «Пропустить».",
                                        reply_markup=skip_button())
        return PAYMENT_PHONE
    return ConversationHandler.END
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== 1. АВТОРИЗАЦИЯ С САЙТА (Вход через Telegram) =====
    ...
    if existing_user[5] == 1:  # first_time == 1
...
async def check_inactive_users(bot: Bot):
    try:
        ...
        c = conn.cursor()
        c.execute('''
            SELECT user_id, start_date, reminder_sent FROM users
            WHERE start_date IS NOT NULL AND subscription_end IS NULL AND reminder_sent < 5
        ''')
        users = c.fetchall()
        conn.close()
        now = datetime.now()
        ...
        for user_id, start_date, reminder_sent in users:
            ...
            if days_passed == reminder_sent:
                ...
                if user_data and user_data[0] is not None:
                    continue
                text = "👋 **Привет!**\n\nВы зарегистрировались в FMH_VPN, но ещё не активировали пробный период.\n\n🎁 Мы дарим вам **3 дня** бесплатного доступа к VPN.\n💡 Чтобы активировать пробный период, просто нажмите /start."
                try:
...
async def check_expiring_subscriptions(bot: Bot):
    try:
        ...
        c.execute('''
            ...
            AND DATEDIFF(subscription_end, NOW()) IN (1, 2, 3)
        ''')
        users = c.fetchall()
        conn.close()
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "help":
        ...
        await update.effective_chat.send_message(
            "📞 Если у вас появились вопросы, напишите нам! Мы ответим в ближайшее время.\nАккаунт для связи: @FMHHELP",
            reply_markup=back_button())
        ...
        user_data = get_user(user_id)
        await update.effective_chat.send_message(
            "💸 **Выберите тариф:**\n\n" f"💰 Ваш бонусный счет: **{user_data[3]} ₽**", parse_mode='Markdown',
        ...
        prices = {"simple": {"1": 249, "3": 599, "6": 999}, "pro": {"1": 499, "3": 1399, "6": 2399}}
        keyboard = [
            [InlineKeyboardButton(f"1 месяц — {prices[tariff]['1']} ₽ (бонусов: {min(bonus, prices[tariff]['1'])} ₽)",
                                  callback_data=f"plan_with_bonus_{tariff}_1")],
            [InlineKeyboardButton(f"3 месяца — {prices[tariff]['3']} ₽ (бонусов: {min(bonus, prices[tariff]['3'])} ₽)",
                                  callback_data=f"plan_with_bonus_{tariff}_3")],
            [InlineKeyboardButton(f"6 месяцев — {prices[tariff]['6']} ₽ (бонусов: {min(bonus, prices[tariff]['6'])} ₽)",
                                  callback_data=f"plan_with_bonus_{tariff}_6")],
            [InlineKeyboardButton("🔙 Выбор тарифа", callback_data="payment_tariff_back")],
            [InlineKeyboardButton("🏠 Главное меню", callback_data="main_menu")],
        ]
        await update.effective_chat.send_message(
            f"📱 **Тариф {tariff.capitalize()}**\n\n💰 Ваш бонусный счет: **{bonus} ₽**\n\nВыберите срок подписки:",
        ...
        keyboard = [
            ...
            [InlineKeyboardButton(f"🔄 Частично бонусами (есть {bonus} ₽)",
                                  callback_data=f"bonus_partial_input_{tariff}_{months}")] if bonus > 0 else [],
        ...
        if bonus >= price:
            ...
            text = f"✅ **Подписка активирована за бонусы!** 🎉\n\n📱 Тариф: {tariff.capitalize()}\n📆 Период: {months} месяц(ев)\n💰 Списано бонусов: **{price} ₽**\n📊 Остаток бонусов: **{bonus - price} ₽**"
        ...
        bonus = user_data[3] if user_data else 0
        if bonus <= 0:
            await update.effective_chat.send_message("❌ У вас нет бонусов для частичной оплаты.",
                                                     reply_markup=back_button())
            return
        context.user_data['selected_tariff'] = tariff
        context.user_data['selected_plan'] = months
        ...
        await update.effective_chat.send_message(
            f"💎 **Частичная оплата бонусами**\n\n💰 Стоимость: **{price} ₽**\n💎 Ваши бонусы: **{bonus} ₽**\n📊 Максимум можно использовать: **{min(bonus, price)} ₽**\n\n✏️ **Введите сумму бонусов, которую хотите потратить:**\n(от 1 до {min(bonus, price)} ₽)",
            parse_mode='Markdown', reply_markup=back_button())
        ...
        payment_data = create_yookassa_payment(user_id=user_id, amount=price, tariff_type=tariff, months=months,
        ...
        if payment_data:
            ...
            await update.effective_chat.send_message(
                f"💳 **Оплата банковской картой**\n\n💰 Сумма: **{price} ₽**\n📱 Тариф: {tariff.capitalize()}\n📆 Период: {months} месяц(ев)\n\n🔗 [Оплатить картой]({payment_data['confirmation_url']})",
                parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(keyboard))
        else:
            await update.effective_chat.send_message("❌ Ошибка создания платежа. Попробуйте позже.",
                                                     reply_markup=back_button())
        ...
        if payment_data:
            ...
            await update.effective_chat.send_message(
                f"🏦 **Оплата через СБП**\n\n💰 Сумма: **{price} ₽**\n📱 Тариф: {tariff.capitalize()}\n📆 Период: {months} месяц(ев)\n\n🔗 [Оплатить через СБП]({payment_data['confirmation_url']})",
                parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(keyboard))
        else:
            await update.effective_chat.send_message("❌ Ошибка создания платежа. Попробуйте позже.",
                                                     reply_markup=back_button())
        ...
        if not payment_id:
            await update.effective_chat.send_message("❌ Платеж не найден. Попробуйте создать новый.",
                                                     reply_markup=back_button())
            return
        ...
        if payment_status and payment_status['paid'] and payment_status['status'] == 'succeeded':
            ...
            if sub_link is not None:
                ...
                if bonus_used > 0 and amount > 0:
                    await update.effective_chat.send_message(
                        f"✅ **Оплата подтверждена!** 🎉\n\n📱 Тариф: {tariff.capitalize()}\n📆 Период: {months} месяц(ев)\n💰 Полная стоимость: **{full_price} ₽**\n💎 Оплачено бонусами: **{bonus_used} ₽**\n💳 Оплачено деньгами: **{amount} ₽**\n🎁 Реферер получил **{int(amount * 0.2)}** бонусов!{link_text}",
            ...
            await update.effective_chat.send_message(
                f"⏳ Платеж еще не оплачен.\nСтатус: {payment_status['status'] if payment_status else 'неизвестен'}\n\nОплатите счет и нажмите «Проверить оплату» снова.",
                reply_markup=back_button())
    elif data == "connect":
        record = get_active_subscription_by_user(user_id)
        if record:
            text = f"🌐 **Ваша ссылка на подписку:**\n{build_subscription_link(record.tariff, record.sub_id)}\n\n<a href='https://quaintly-ornate-basil.tilda.ws/'>Инструкция по подключению</a>"
        else:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_captcha and 'answer' in user_captcha[user_id]:
        try:
            ...
            if user_answer == correct_answer:
                ...
                await update.message.reply_text("✅ **Капча успешно пройдена!** Добро пожаловать в FMH-VPN!",
                                                parse_mode='Markdown')
    ...
    if context.user_data.get('awaiting_bonus_input'):
        try:
            ...
            if remaining > 0:
                payment_data = create_yookassa_payment(user_id=user_id, amount=remaining, tariff_type=tariff,
                                                       months=months,
                ...
                if payment_data:
                    ...
                    await update.message.reply_text(
                        f"✅ **Частичная оплата**\n\n💰 Полная стоимость: **{full_price} ₽**\n💎 Списано бонусов: **{bonus_amount} ₽**\n💳 Остаток к оплате: **{remaining} ₽**\n\n🔗 [Оплатить остаток]({payment_data['confirmation_url']})",
                        parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(keyboard))
                else:
                    conn = get_db_connection()
                    c = conn.cursor()
                    ...
                    conn.commit()
                    conn.close()
                    await update.message.reply_text("❌ Ошибка создания платежа. Бонусы возвращены.",
                                                    reply_markup=back_button())
            else:
                sub_link = apply_subscription_payment(user_id, tariff, days=30 * months)
                text_msg = f"✅ **Подписка полностью оплачена бонусами!** 🎉\n\n📱 Тариф: {tariff.capitalize()}\n📆 Период: {months} месяц(ев)\n💰 Списано бонусов: **{bonus_amount} ₽**"
                text_msg += f"\n\n🔑 **Ваша ссылка на подписку:**\n`{sub_link}`" if sub_link else "\n\n⚠️ Не удалось выдать ключ автоматически, напишите в поддержку."
                await update.message.reply_text(text_msg, parse_mode='Markdown', reply_markup=main_menu())
        except ValueError:
